...
public class UserDAO {
    ...
    static final String USER_COLUMNS = "UserID, Username, Email, PasswordHash, FullName, Phone, "
            + "Address, IdentityNumber, Status, CreatedAt";
    /**
    ...
    public boolean registerCustomer(User user) {
        String insertUser = "INSERT INTO dbo.Users "
                + "(Username, Email, PasswordHash, FullName, Phone, Address, IdentityNumber) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?)";
    ...
    public boolean createUser(User user, List<String> roleNames) {
        String insertUser = "INSERT INTO dbo.Users "
                + "(Username, Email, PasswordHash, FullName, Phone, Address, IdentityNumber, Status) "
                + "VALUES (?, ?, ?, ?, ?, ?, ?, ?)";
        String assignRole = "INSERT INTO dbo.User_Roles (UserID, RoleID) "
        ...
        try (Connection conn = DBContext.getConnection()) {
            ...
            try {
                ...
                if (roleNames.contains("CUSTOMER")) {
                    try (PreparedStatement ps = conn.prepareStatement(
                            "IF NOT EXISTS (SELECT 1 FROM dbo.Customers WHERE UserID = ?) "
                            + "INSERT INTO dbo.Customers (UserID) VALUES (?)")) {
                        ps.setInt(1, userId);
                        ps.setInt(2, userId);
                        ps.executeUpdate();
                ...
                if (roleNames.contains("DRIVER")) {
                    try (PreparedStatement ps = conn.prepareStatement(
                            "IF NOT EXISTS (SELECT 1 FROM dbo.Drivers WHERE UserID = ?) "
                            + "INSERT INTO dbo.Drivers "
                            + "(UserID, LicenseNumber, LicenseClass, LicenseExpiryDate) "
                            + "VALUES (?, ?, N'B2', DATEADD(YEAR, 5, GETDATE()))")) {
                        ps.setInt(1, userId);
                        ps.setInt(2, userId);
                        ps.setString(3, "DL-" + userId);
                        ps.executeUpdate();
                    }
                }
                conn.commit();
