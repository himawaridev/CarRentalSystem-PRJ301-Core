...
<div class="container py-5">
    <div class="d-flex justify-content-between align-items-center mb-4">
        ...
        <a href="${pageContext.request.contextPath}${staffView ? '/staff/dashboard' : '/my-contracts'}" class="btn btn-outline-accent btn-sm">
    ...
    <div class="card-custom">
        <div class="card-body">
            <div class="d-flex flex-wrap gap-2 align-items-center">
                <span class="badge ${contract.status == 'PENDING_PAYMENT' || contract.status == 'RESERVED' || contract.status == 'CONFIRMED' || contract.status == 'CAR_PICKED_UP' || contract.status == 'CAR_RETURNED' || contract.status == 'COMPLETED' ? 'bg-primary' : 'bg-secondary'} rounded-pill px-3 py-2">
                    <i class="bi bi-send me-1"></i>Tao don
                </span>
                <i class="bi bi-chevron-right text-muted"></i>
                <span class="badge ${contract.status == 'RESERVED' || contract.status == 'CONFIRMED' || contract.status == 'CAR_PICKED_UP' || contract.status == 'CAR_RETURNED' || contract.status == 'COMPLETED' ? 'bg-primary' : 'bg-secondary'} rounded-pill px-3 py-2">
                    <i class="bi bi-credit-card me-1"></i>Thanh toan coc
                </span>
                <i class="bi bi-chevron-right text-muted"></i>
                <span class="badge ${contract.status == 'CONFIRMED' || contract.status == 'CAR_PICKED_UP' || contract.status == 'CAR_RETURNED' || contract.status == 'COMPLETED' ? 'bg-primary' : 'bg-secondary'} rounded-pill px-3 py-2">
                    <i class="bi bi-check-circle me-1"></i>Xac nhan
                </span>
                <i class="bi bi-chevron-right text-muted"></i>
                <span class="badge ${contract.status == 'CAR_PICKED_UP' || contract.status == 'CAR_RETURNED' || contract.status == 'COMPLETED' ? 'bg-primary' : 'bg-secondary'} rounded-pill px-3 py-2">
                    <i class="bi bi-car-front me-1"></i>Nhan xe
                </span>
                <i class="bi bi-chevron-right text-muted"></i>
                <span class="badge ${contract.status == 'CAR_RETURNED' || contract.status == 'COMPLETED' ? 'bg-primary' : 'bg-secondary'} rounded-pill px-3 py-2">
                    <i class="bi bi-arrow-return-left me-1"></i>Tra xe
                </span>
                <i class="bi bi-chevron-right text-muted"></i>
                <span class="badge ${contract.status == 'COMPLETED' ? 'bg-primary' : 'bg-secondary'} rounded-pill px-3 py-2">
